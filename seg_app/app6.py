...
def main():
    ...
    if option == 'Click':
        ...
        if st.session_state['image'] is not None:
            h, w = st.session_state['image'].size
            ...
            with col1:
                ...
                # Create canvas based on fore_back selection
            ...
            with col2:
                ...
                pos_points = np.array(pos_points) if pos_points else np.array([])
                neg_points = np.array(neg_points) if neg_points else np.array([])

                if sam_button and (len(pos_points) > 0 or len(neg_points) > 0):
                    generator, predictor = get_sam()
                    set_pred_img(predictor, np.array(st.session_state['image']))
                    
                    input_point = np.concatenate([pos_points, neg_points], axis=0) if len(pos_points) > 0 and len(neg_points) > 0 else (pos_points if len(pos_points) > 0 else neg_points)
                    ...
                    fig, ax = plt.subplots(figsize=(20,20))
                    ax.imshow(st.session_state['image'])
                    show_points(pos_points, neg_points, ax)
                    ax.axis('off')
                    st.pyplot(fig)
